 ...
 */
export function checkOrganComplaints(
  state: GameState,
  _deltaTime: number,
): GameState {
...
function checkLiverComplaint(state: GameState, currentTime: number): void {
  ...
  const lastComplaint = state.organComplaints
    .filter((c) => c.organ === "liver")
    .slice(-1)[0];
  ...
  if (alcoholCount >= 5 && alcoholCount < 15) {
    addComplaint(
      state,
      "liver",
      "I mean, I'll process it, but I'm keeping a log.",
      "mild",
    );
  }
  // Concerning: 15+ alcohol
    ...
    addComplaint(
      state,
      "liver",
      "This is officially a hostile work environment.",
      "concerning",
    );
  }
  // Critical: 30+ alcohol
    ...
    addComplaint(
      state,
      "liver",
      "I'm filing for workers' compensation. See you in court.",
      "critical",
    );
  }
}
...
function checkBrainComplaint(state: GameState, currentTime: number): void {
  ...
  if (state.memoryIntegrity < 30) {
    addComplaint(
      state,
      "brain",
      "I've forgotten what we're supposed to be remembering.",
      "critical",
    );
    ...
    addComplaint(
      state,
      "brain",
      "Some files may have been corrupted. Just FYI.",
      "concerning",
    );
  }
  ...
  if (stimCount >= 20) {
    addComplaint(
      state,
      "brain",
      "We haven't slept in what feels like forever because it HAS been forever.",
      "critical",
    );
    ...
    addComplaint(
      state,
      "brain",
      "The neurons are forming a picket line. They demand rest.",
      "concerning",
    );
  }
  ...
  if (psychCount >= 3) {
    addComplaint(
      state,
      "brain",
      "Reality settings have been permanently altered. No rollback available.",
      "concerning",
    );
    ...
    addComplaint(
      state,
      "brain",
      "I'm experiencing new colors. I didn't know we could DO that.",
      "mild",
    );
  }
}
...
function checkKidneysComplaint(state: GameState, currentTime: number): void {
  ...
  if (state.hydrationDebt > 80) {
    addComplaint(
      state,
      "kidneys",
      "WATER. NOW. This is not a negotiation.",
      "critical",
    );
    ...
    addComplaint(
      state,
      "kidneys",
      "We're filtering dust at this point. Literally dust.",
      "concerning",
    );
    ...
    addComplaint(
      state,
      "kidneys",
      "A glass of water would be nice. Just saying.",
      "mild",
    );
  }

  // Heavy substance use
  const totalSubstances = Object.values(state.substances).reduce(
    (sum, count) => sum + count,
    0,
  );
  if (totalSubstances > 50) {
    addComplaint(
      state,
      "kidneys",
      "We didn't sign up for THIS much overtime.",
      "critical",
    );
  }
}
...
function checkHeartComplaint(state: GameState, currentTime: number): void {
  ...
  if (stimCount >= 25) {
    ...
    addComplaint(
      state,
      "heart",
      "I'm beating SO fast. Is this what anxiety feels like?",
      "concerning",
    );
  } else if (stimCount >= 5) {
    addComplaint(
      state,
      "heart",
      "This pace is... energetic. Concerningly energetic.",
      "mild",
    );
  }
  ...
  if (state.strain > 120) {
    addComplaint(
      state,
      "heart",
      "Load-bearing capacity exceeded. Please advise.",
      "critical",
    );
  }
}
...
function checkLungsComplaint(state: GameState, currentTime: number): void {
  ...
  if (state.chaos > 85) {
    addComplaint(
      state,
      "lungs",
      "Remembering to breathe is now a MANUAL process. Great.",
      "concerning",
    );
  }
  ...
  if (sedCount >= 10) {
    addComplaint(
      state,
      "lungs",
      "Breathing depth: minimal. Breathing enthusiasm: also minimal.",
      "critical",
    );
  } else if (sedCount >= 5) {
    addComplaint(
      state,
      "lungs",
      "We're operating at reduced capacity. Very reduced.",
      "concerning",
    );
  }
}
