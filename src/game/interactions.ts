...
export const INTERACTION_MATRIX: Record<
  string,
  Record<string, SubstanceInteraction>
> = {
  alcohol: {
    sedative: {
...
export function getActiveSubstanceTypes(
  ...
  for (let i = 0; i < activeTypes.length; i++) {
    for (let j = i + 1; j < activeTypes.length; j++) {
      ...
      if (interaction) {
        ...
        if (
          interaction.specialEffect &&
          !specialEffects.includes(interaction.specialEffect)
        ) {
