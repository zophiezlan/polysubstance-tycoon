...
export const ENERGY_MODES: Record<string, EnergyMode> = {
  balanced: {
    ...
    name: "Baseline",
    description:
      "Standard energy mechanics. Earn energy from actions, spend it on powerful effects.",
...
export const ENERGY_BOOSTERS: Record<string, EnergyBooster> = {
  ...
  powerSurge: {
    ...
    name: "💥 Power Surge",
    description:
      "Spend 40 energy for +5x click power AND +2x production for 15 seconds.",
    cooldown: 0,
    energyCost: 40,
  ...
  zenState: {
    ...
    name: "🧘 Zen State",
    description:
      "Spend 30 energy to balance chaos at 50. Gain +3x production for 20 seconds.",
    cooldown: 0,
    energyCost: 30,
    ...
    apply: (state: GameState) => {
      ...
      state.log.push({
        timestamp: state.timeRemaining,
        message:
          "🧘 Zen State achieved! Chaos balanced, +3x production for 20s",
        type: "achievement",
      });
    },
  },
...
export function getAvailableEnergyModes(state: GameState): EnergyMode[] {
  return Object.values(ENERGY_MODES).filter((mode) =>
    mode.unlockCondition(state),
  );
}

export function canUseEnergyBooster(
...
export function useEnergyBooster(
  state: ExtendedGameState,
  boosterId: string,
): boolean {
...
export function switchEnergyMode(
  state: ExtendedGameState,
  modeId: string,
): boolean {
...
export function getEnergyProductionMultiplier(
  state: ExtendedGameState,
): number {
  const mode = getActiveEnergyMode(state);
