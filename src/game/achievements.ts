...
export const ACHIEVEMENTS: Achievement[] = [
  // Early Game
  {
    ...
    checkCondition: (state) =>
      state.nightsCompleted >= 0 && state.isNightActive,
  },
  {
    id: "justACouple",
  ...
  // Mid Game
  {
    ...
    checkCondition: (state) => {
      const activeTypes = Object.keys(state.substances).filter(
  ...
  // Late Game / Cursed
  {
    ...
    checkCondition: (state) => {
      const collapseThreshold =
        100 + state.energy * 0.5 - state.hydrationDebt * 0.3;
  ...
  },
  {
    ...
    checkCondition: (state) =>
      !state.isNightActive && state.memoryIntegrity < 10 && !state.hasCollapsed,
  },
  {
    id: "unbreakable",
    name: "Unbreakable",
  ...
  },
  {
    ...
    description:
      "Complete a night with max Vibes AND no Collapse using only maintenance strategies",
    hidden: true,
    checkCondition: () => false, // Checked separately in game logic
  },
  ...
  },
  {
    ...
    name: "Click Ascended",
    description:
      "Click 1,000,000 times. Your fingers are beyond mortal comprehension.",
  ...
  },
  {
    ...
    description:
      "Own all basic synergy upgrades (Cocktail, Speedball, Candy Flip)",
    checkCondition: (state) => {
      return (
        state.upgrades.includes("cocktail-theory") &&
  ...
  },
  {
    ...
    name: "Polypharmacy",
    description:
      "Have 5+ substance types active with at least one synergy bonus",
    checkCondition: (state) => {
      const activeTypes = Object.keys(state.substances).filter(
        (id) => state.substances[id] > 0,
      );
      const hasSynergy = state.upgrades.some((id) =>
        [
          "cocktail-theory",
          "speedball-dynamics",
          "candy-flip-protocol",
  ...
  },
  {
    ...
    name: "Idle Tycoon",
    description:
      "Earn 100,000 vibes without clicking (auto-clicker + passive only)",
  ...
  },
  {
    ...
    checkCondition: (state) => {
      return (
        state.upgrades.includes("test-kit-pro") &&
