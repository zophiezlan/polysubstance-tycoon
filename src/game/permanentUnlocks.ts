...
export const PERMANENT_UNLOCKS: Record<string, PermanentUnlock> = {
  ...
  hotkey_controls: {
    ...
    description:
      "Enable keyboard shortcuts for common actions (1-5 for energy boosters, Q/W/E for chaos actions).",
    insightCost: 2,
    category: "quality-of-life",
  },
  ...
  extra_build_slot_1: {
    ...
    unlockCondition: (state) =>
      state.unlockedFeatures.includes("build_presets"),
    maxPurchases: 1,
  },
  ...
  auto_purchase_substances: {
    ...
    description:
      "Automatically purchase substances when you have enough vibes. Configure which ones in settings.",
    insightCost: 5,
    category: "automation",
  },
  ...
  auto_use_maintenance: {
    ...
    description:
      "Automatically use maintenance actions when meters get critical (energy < 20, chaos > 80).",
    insightCost: 7,
    category: "automation",
  },
  ...
  auto_clicker_boost: {
    ...
    description:
      "Increase all auto-clicker speeds by +50%. Stackable up to 5 times.",
    insightCost: 3,
    category: "automation",
  ...
  fresh_start_bonus: {
    ...
    description:
      "Start each prestige with 25% of your previous substances. Stackable.",
    insightCost: 5,
    category: "special",
...
export function getAvailablePermanentUnlocks(
...
export function purchasePermanentUnlock(
  ...
  state.permanentUnlocks[unlockId] =
    (state.permanentUnlocks[unlockId] || 0) + 1;
...
export function getPermanentUnlockLevel(
  state: ExtendedGameState,
  unlockId: string,
): number {
...
export function hasPermanentUnlock(
  state: ExtendedGameState,
  unlockId: string,
): boolean {
...
export function getPermanentProductionMultiplier(
  ...
  const boostLevel = getPermanentUnlockLevel(
    state,
    "permanent_production_boost",
  );
...
export function getAutoClickerSpeedMultiplier(
  state: ExtendedGameState,
): number {
...
export function getChaosActionCooldownMultiplier(
  state: ExtendedGameState,
): number {
