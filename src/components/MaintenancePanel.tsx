...
export function MaintenancePanel({ state, onAction }: MaintenancePanelProps) {
  ...
  const suppressWarnings =
    state.confidence > 70 || (state.substances.sedative || 0) >= 3;
  ...
  const formatEffects = (action: (typeof MAINTENANCE_ACTIONS)[0]) => {
    ...
    if (action.effects.energyRestore)
      effects.push(`⚡ +${action.effects.energyRestore} Energy`);
    if (action.effects.chaosReduction)
      effects.push(`🌪️ -${action.effects.chaosReduction} Chaos`);
    if (action.effects.strainReduction)
      effects.push(`💪 -${action.effects.strainReduction} Strain`);
    if (action.effects.hydrationRestore)
      effects.push(`💧 -${action.effects.hydrationRestore} Dehydration`);
    if (action.effects.memoryRestore)
      effects.push(`🧠 +${action.effects.memoryRestore} Memory`);
    if (action.effects.timeBonus)
      effects.push(`⏱️ +${action.effects.timeBonus}s Time`);
    if (action.effects.sleepDebtReduction)
      effects.push(`😴 -${action.effects.sleepDebtReduction} Sleep Debt`);
    return effects;
  };
  ...
  return (
    <div className="maintenance-panel">
      ...
      <p className="maintenance-description">
        Keep yourself running throughout the night
      </p>
      <div className="maintenance-actions">
        {MAINTENANCE_ACTIONS.map((action) => {
          ...
          return (
            <div
              key={action.id}
              className={`maintenance-action ${disabled ? "disabled" : "available"}`}
            >
              <button
                ...
                {onCooldown && (
                  <div className="cooldown-bar-container">
                    <div
                      ...
                      style={{
                        width: `${(cooldownRemaining / action.cooldown) * 100}%`,
                      }}
                    />
                  </div>
                )}
